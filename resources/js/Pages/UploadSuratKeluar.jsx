...
export default function UploadSuratKeluar({ auth, surat_keluar }) {
    const [values, setValues] = useState({
        id: surat_keluar.id ?? "",
        number: surat_keluar.number ?? "",
        name: surat_keluar.name ?? "",
        date: surat_keluar.date ?? "",
        from: surat_keluar.from ?? "",
        category: surat_keluar.category ?? "Surat Keputusan",
        description: surat_keluar.description ?? "",
    ...
    function handleSubmit(e) {
        e.preventDefault()
        router.post('/upload-surat-keluar', values)
    }
    ...
    useEffect(() => {
        async function addToFile() {
            ...
            const fileToAttach = surat_keluar.file && await imageUrlToFile(surat_keluar.file);
            const dataTransfer = new DataTransfer();
            dataTransfer.items.add(fileToAttach);
            fileInputElement.files = dataTransfer.files;
        }

        surat_keluar.file && addToFile();
    }, [])
    ...
    return(
        ...
        >
            ...
            >
                ...
                <p className='text-sm font-montserrat ml-14'>Tambah Surat Keluar Sistem Informasi Surat Menyurat LIMPAKO</p>
